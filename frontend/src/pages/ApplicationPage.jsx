    ...
    <div>
        ...
        <div className="flex items-center gap-4 rounded-lg border border-dashed border-navy-900/20 p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-navy-900">
              {person.photoName || "Upload official photo"}
            </p>
          ...
          <label className="cursor-pointer rounded-lg border border-navy-900/15 px-3 py-1.5 text-xs font-semibold text-navy-900 hover:bg-navy-900/5">
            ...
            <input
              ...
              className="hidden"
              onChange={(e) => setPerson({ ...person, photoName: e.target.files?.[0]?.name || "" })}
            />
          </label>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <Field
          ...
          onChange={(v) => setPerson({ ...person, studentId: v })}
        />
        <Field
          label="Department"
          placeholder="e.g. Computer Science"
          value={person.department}
          onChange={(v) => setPerson({ ...person, department: v })}
        />
        <Field
          label="Year of Study"
          placeholder="e.g. 4th Year"
          value={person.year}
          onChange={(v) => setPerson({ ...person, year: v })}
        />
        <Field
  ...
  if (submitted) {
    return (
      <div className="mx-auto max-w-lg px-6 py-20 text-center">
        ...
        <h1 className="mt-4 font-display text-xl font-semibold text-navy-900">Application submitted</h1>
    ...
    <div>
      ...
      <div className="mx-auto max-w-2xl px-6 py-8">
        ...
        <div className="mt-8 rounded-2xl border border-navy-900/10 bg-white p-6 shadow-card sm:p-8">
          ...
          {step === 4 && (
            <div>
              <h1 className="font-display text-lg font-semibold text-navy-900">Review your application</h1>
              <p className="mt-1.5 text-sm text-navy-900/60">
                Double-check everything below before submitting — you can still go back to edit.
              </p>
        ...
        <p className="mt-4 text-center text-xs text-navy-900/40">
          Need help? elections@erica.hanyang.ac.kr
        </p>
      </div>
    </div>
  );
